...
- **Camera ceiling feeling more restrictive than today's actual range**: `EXTERIOR_RING_WIDTH` (14) is chosen to keep `maxOrbitRadius` comfortably above the current system's existing practical max (today's `MAX_ZOOM_OFFSET`/`ORBIT_RADIUS_PER_ZONE` ceiling). Verified post-implementation: the cap (`24 + 2.5·zones`) stays above the existing zoom-out limit (`15 + 3·zones`) at every zone count from 0 to 4, meaning the cap is currently a redundant future-proofing guard rather than today's binding constraint — the actual "no visible void" guarantee comes from `GymExterior`'s large ground plane. If a future change to `MAX_ZOOM_OFFSET` or the zone system ever lets the camera zoom out further than today, this cap is what prevents it from seeing past the exterior scenery.
